...
def bot_message(message):
    ...
    if message.chat.type == 'private':
        ...
        if message.text == '💵Курсы валют':
            ...
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            item1 = types.KeyboardButton('👀Посмотреть погоду')
            back = types.KeyboardButton('🔙Назад')
            markup.add(item1, back)
            bot.send_message(message.chat.id, "{0.first_name}, вы открыли раздел «⛅Погода»!".format(message.from_user),
                             reply_markup=markup)
            ...
            def showWeather(message):
                ...
                try:
                    ...
                    if weather_description in get_weather.code_to_emoji:
                        wd = get_weather.code_to_emoji[weather_description]
                    ...
                    bot.send_message(message.chat.id, str(text), reply_markup=markup)

                    if message != city:
                        bot.send_message(message.chat.id, "Не нашел информации по вашему запросу😭")
            ...
            bot.send_message(message.chat.id,
                             "{0.first_name},вы открыли раздел «🌎Новости Мира»!".format(message.from_user),
                             reply_markup=markup)

        elif message.text == '🇰🇿 Новости Казахстана':
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            bot.send_message(message.chat.id,
                             "{0.first_name},вы открыли раздел «🇰🇿 Новости Казахстана»!".format(message.from_user),
                             reply_markup=markup)
